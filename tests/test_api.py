...
from homecredit_service.schemas import MAX_BATCH_RECORDS
...
def test_api_predict_and_metadata(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        importance = client.get("/feature-importance?limit=5")
        assert importance.status_code == 200
        assert len(importance.json()) <= 5
...
def test_api_validation_errors_and_service_unavailable(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        too_large_batch = client.post(
            "/predict/batch",
            json={
                "records": [{"AMT_INCOME_TOTAL": 180000}] * (MAX_BATCH_RECORDS + 1),
                "top_n": 3,
            },
        )
