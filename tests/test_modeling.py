...
def test_training_applies_allowed_hyperparameter_overrides() -> None:
    rng = np.random.default_rng(222)
    frame = pd.DataFrame(
        {
            "SK_ID_CURR": np.arange(240),
            "TARGET": rng.binomial(1, 0.1, size=240),
            "AMT_INCOME_TOTAL": rng.normal(180000, 20000, size=240),
            "AMT_CREDIT": rng.normal(600000, 50000, size=240),
            "AMT_ANNUITY": rng.normal(25000, 3000, size=240),
            "DAYS_BIRTH": -rng.integers(9000, 25000, size=240),
            "CODE_GENDER": rng.choice(["M", "F"], size=240),
        }
    )
    ...
    assert bundle["hyperparameters"]["n_estimators"] == 80
    assert bundle["hyperparameters"]["learning_rate"] == 0.04
    assert bundle["hyperparameters"]["num_leaves"] == 32
    assert bundle["hyperparameters"]["random_state"] == 222
