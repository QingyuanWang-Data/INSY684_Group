...
from homecredit_service.tracking import log_training_run_to_mlflow
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable-mlflow",
        action="store_true",
        help="Log training parameters, metrics, and artifacts to MLflow.",
    )
    parser.add_argument(
        "--mlflow-tracking-uri",
        default=None,
        help="Optional MLflow tracking URI. Defaults to MLflow's local ./mlruns behavior.",
    )
    parser.add_argument(
        "--mlflow-experiment",
        default="insy684-homecredit",
...
def main() -> None:
    ...
    if args.enable_mlflow:
        ...
        if run_id is not None:
            logger.info("MLflow run logged with run_id=%s", run_id)
