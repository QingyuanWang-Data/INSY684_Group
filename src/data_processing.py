...
def build_data_profile(df: pd.DataFrame) -> pd.DataFrame:
    """Create a column-level data profiling summary for the modeling frame."""
    ...
    return profile.sort_values("missing_rate", ascending=False)


def run_data_quality_checks(df: pd.DataFrame) -> dict:
    """Return basic data quality checks for the final training frame."""
    checks = {
        "row_count": int(len(df)),
        "column_count": int(df.shape[1]),
        "duplicate_applicants": int(df[ID_COLUMN].duplicated().sum()),
    }

    if TARGET_COLUMN in df.columns:
        checks["target_missing"] = int(df[TARGET_COLUMN].isna().sum())
        checks["positive_count"] = int((df[TARGET_COLUMN] == 1).sum())
        checks["negative_count"] = int((df[TARGET_COLUMN] == 0).sum())
