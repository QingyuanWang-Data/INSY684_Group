...
def subgroup_frame(training_report: dict[str, Any], split: str) -> pd.DataFrame:
    """Convert subgroup metrics embedded in training_report.json to table rows."""
...
def render_policy(training_report: dict[str, Any], artifact_dir: Path) -> None:
    ...
    if cost_frame.empty:
        cost_rows = training_report.get("cost_sensitivity", {}).get("rows", [])
...
def render_fairness(training_report: dict[str, Any], artifact_dir: Path) -> None:
    validation = load_csv(artifact_dir / "validation_subgroup_metrics.csv")
    test = load_csv(artifact_dir / "test_subgroup_metrics.csv")
    if validation.empty:
        validation = subgroup_frame(training_report, "validation")
    if test.empty:
        test = subgroup_frame(training_report, "test")
...
def main() -> None:
    ...
    with tabs[4]:
        render_fairness(training_report, artifact_dir)
    with tabs[5]:
