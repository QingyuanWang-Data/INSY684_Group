...
def policy_section(training_report: dict[str, Any], artifact_dir: Path) -> str:
    ...
    cost_frame = load_csv(artifact_dir / "cost_sensitivity.csv")
    if cost_frame.empty:
        cost_frame = pd.DataFrame(training_report.get("cost_sensitivity", {}).get("rows", []))
    return f"""
    <section>
      <h2>Policy Monitoring</h2>
      <div class="metric-grid">{metrics}</div>
      <h3>Cost Sensitivity by Threshold</h3>
      {table_html(cost_frame)}
    </section>
    """


def embedded_subgroup_frame(training_report: dict[str, Any], split: str) -> pd.DataFrame:
    payload = training_report.get("subgroup_performance", {}).get(split, {})
    columns = payload.get("columns", {}) if isinstance(payload, dict) else {}
    rows: list[dict[str, Any]] = []
    for column, details in columns.items():
        for group in details.get("groups", []):
            rows.append({"column": column, **group})
    return pd.DataFrame(rows)


def fairness_section(training_report: dict[str, Any], artifact_dir: Path) -> str:
    validation = load_csv(artifact_dir / "validation_subgroup_metrics.csv")
    test = load_csv(artifact_dir / "test_subgroup_metrics.csv")
    if validation.empty:
        validation = embedded_subgroup_frame(training_report, "validation")
    if test.empty:
        test = embedded_subgroup_frame(training_report, "test")
    return f"""
    <section>
...
def build_html(artifact_dir: Path) -> str:
    ...
    if not drift_summary:
        embedded_drift = training_report.get("drift_summary", {})
        drift_summary = embedded_drift if isinstance(embedded_drift, dict) else {}
...
<body>
  ...
  <main>
    ...
    {fairness_section(training_report, artifact_dir)}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "artifacts" / "monitoring_dashboard.html",
    )
